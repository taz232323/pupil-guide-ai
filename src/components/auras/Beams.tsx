...
function extendMaterial(BaseMaterial: any, cfg: any) {
  const physical: any = (THREE as any).ShaderLib.physical;
...
const DirLight = ({ position, color }: { position: [number, number, number]; color: string }) => {
  ...
  useEffect(() => {
    ...
    const cam: any = dir.current.shadow.camera;
    if (!cam) return;
...
const Beams = (props: BeamsProps) => (
  <div className="absolute inset-0 h-full w-full">
    <Canvas dpr={[1, 2]} frameloop="always" gl={{ antialias: true, alpha: true }} camera={{ fov: 30, position: [0, 0, 20] }}>
